...
#6.构建全部完成后触发发布脚本(254本地机房服务器)
#=============正文如下===========================正文如下=================正文如下==========================正文如下===============#
# 设置工作目录/构建目录/执行目录/部署目录
work_dir="/home/jenkins/build"
build_dir="$work_dir/bbx-saas/build/beta/uat_fat"
execute_dir="/home/jenkins/execute"
target_dir="$execute_dir/beta/uat_fat"
...
chmod +x $build_dir/deploy-beta.sh
set -e
...
build_app() {
    ...
    cp -f "$build_dir/load-env.sh" "$target_dir"
    cp -f "$build_dir/deploy-beta.sh" "$target_dir"
    sh -v "$target_dir/deploy-beta.sh"
}
...
# 如果提交格式不符合要求，直接退出
if ! [[ $commit_message =~ ^\[build-(rc|public)\]([a-zA-Z0-9-]+)-([a-zA-Z0-9-]+)--(v[0-9]+\.[0-9]+\.[0-9]+)$ ]]; then
    echo "提交格式有误 错误格式为: $commit_message"
    sendDingDing "### **通报: 提交格式不符合要求**\n#### $NIU:\n- $presenter错误提交\n- 提交记录为$commit_message\n- 正确格式为: [build-env]item-app-version\n- 本次构建失败并退出"
    exit 1
fi
    ...
    rc|public)
        case $app in
            account|content|finance|project|iam|marketing|communal|operation|mall)
                if ! [[ $commit_message =~ ^\[build-(rc|public)\]main-[a-zA-Z0-9-]+--v[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
                    echo "提交格式不符合要求: $commit_message"
                    sendDingDing "### **通报: 提交格式不符合要求**\n#### $NIU:\n- $presenter错误提交\n- 提交记录:$commit_message\n- 正确格式: [build-env]item-app-version\n- 本次构建失败并退出"
                    exit 1
                fi
                app_dir="$work_dir/bbx-saas/app/main/$app"
                ;;
            im|infra)
                if ! [[ $commit_message =~ ^\[build-(rc|public)\]common-[a-zA-Z0-9-]+--v[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
                    echo "提交格式不符合要求: $commit_message"
                    sendDingDing "### **通报: 提交格式不符合要求**\n#### $NIU:\n- $presenter错误提交\n- 提交记录为$commit_message\n- 正确格式: [build-env]item-app-version\n- 本次构建失败并退出"
                    exit 1
                fi
                ...
                ;;
            admin|interface|web)
                if ! [[ $commit_message =~ ^\[build-(rc|public)\](bbx|bbz)-[a-zA-Z0-9-]+--v[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
                    echo "提交格式不符合要求: $commit_message"
                    sendDingDing "### **通报: 提交格式不符合要求**\n#### $NIU:\n- $presenter错误提交\n- 提交记录为$commit_message\n- 正确格式: [build-env]item-app-version\n- 本次构建失败并退出"
                    exit 1
                fi
