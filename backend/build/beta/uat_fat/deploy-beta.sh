...
#=============正文如下===========================正文如下=================正文如下==========================正文如下===============#
# 设置工作目录 # 生效本地变量  生效传参变量  生效钉钉通知
execute_dir="/home/jenkins/execute"
source "$execute_dir/local-info.sh"
source "$execute_dir/beta/uat_fat/load-env.sh"
source "$execute_dir/deploy-notice.sh"
itemapp="${item}${app}"
...
sendMessages() {
    logs=$(docker logs "$environ-$item-$app-$version" 2>&1 | grep "server listening on" | sed 's/"/\\"/g' | sed ':a;N;$!ba;s/\n/\\n/g')
...
docker_common() {
    ...
    if [ -n "$port" ] && [ -n "$port_two" ]; then
        # 如果有两个端口
        portOption="-p $port:${!itemapp} -p $port_two"
    elif [ -n "$port" ]; then
        # 如果只有一个端口
    ...
    # 运行 Docker 容器
    docker run -di --name "$environ-$item-$app-$version" $portOption \
        -v "$volumedir/$environ-$item-$app/logs:/app-acc/logs" \
        -v "$volumedir/$environ-$item-$app:/app-acc/configs" \
        -e BBX_SAAS_RUNTIME_ENV="$environ" --restart always "$repo_addr/$environ-$item-$app:$version"
    ...
    public)
        case $app in
            admin|interface|web)
                case $item in
                    bbx)
                        docker_run "54${!itemapp:0:3}"
                        ;;
                    bbz)
                        docker_run "57${!itemapp:0:3}"
                        ;;
                esac
                ;;
            im)
                docker_run "545${!itemapp:0:2}" "10004:10004"
                ;;
            *)
                docker_no_port
                ;;
    ...
    rc)
        case $app in
            admin|interface|web)
                case $item in
                    bbx)
                        docker_run "541${!itemapp:0:2}"
                        ;;
                    bbz)
                        docker_run "571${!itemapp:0:2}"
                        ;;
                esac
                ;;
            im)
                docker_run "5457${!itemapp:0:1}" "11004:10004"
                ;;
            *)
                docker_no_port
                ;;
